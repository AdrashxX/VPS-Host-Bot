...
<body class="flex flex-col min-h-screen">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-[#111827] border border-[#1f2937] p-5 rounded-2xl space-y-4">
                        <div class="flex justify-between items-center">
                            ...
                            <button onclick="refreshProjects()" class="text-xs px-3 py-1.5 bg-[#1f293d] hover:bg-[#2d3748] rounded-xl font-semibold transition">🔄 reload list</button>
                        </div>
    ...
    <script>
            ...
            projects.forEach(p => {{
                ...
                el.innerHTML = `
                    ...
                    <div class="border-t border-[#1f2937]/50 pt-4 flex flex-wrap gap-2 text-xs font-semibold">
                        ...
                        <button onclick="openModal('logs', ${{p.id}})" class="px-3.5 py-2 bg-[#1f293d] hover:bg-[#2d3748] rounded-xl transition">📋 Live Logs</button>
                        <button onclick="triggerControl(${{p.id}}, 'build')" class="px-3.5 py-2 bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-400 border border-yellow-500/20 rounded-xl transition">🧹 Rebuild</button>
                        <button onclick="triggerControl(${{p.id}}, 'purge')" class="px-3.5 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 rounded-xl transition">🗑️ Purge</button>
                    </div>
                `;
                container.appendChild(el);
            }});
        ...
        function triggerControl(pId, action) {{
            if (action === 'purge' && !confirm('WARNING: This will stop and permanently delete this workspace folder. Continue?')) return;
            fetch('/api/project/control', {{
                method: 'POST',
                headers: {{ 'Content-Type': 'application/json' }},
                body: JSON.stringify({{ project_id: pId, action: action }})
            }})
            .then(r => r.json())
            .then(data => {{
        ...
        // Modal triggers
        function openModal(modalId, pId) {{
            ...
            if (modalId === 'env') {{
                ...
                try {{
                    const parsed = JSON.parse(proj.env_vars || '{{}}');
                    document.getElementById('modal-env-json').value = JSON.stringify(parsed, null, 2);
                }} catch(e) {{
                    document.getElementById('modal-env-json').value = '{{}}';
                }}
            }} else if (modalId === 'logs') {{
                fetchActiveLogs();
            }}
        }}
        ...
        function saveProjectEnv() {{
            const jsonText = document.getElementById('modal-env-json').value;
            let parsed = {{}};
            try {{
                parsed = JSON.parse(jsonText);
            }} catch(e) {{
                alert('Invalid JSON Schema. Operation rejected.');
                return;
            }}
            fetch('/api/project/env', {{
                method: 'POST',
                headers: {{ 'Content-Type': 'application/json' }},
                body: JSON.stringify({{ project_id: activeModalProjectId, env_vars: parsed }})
            }})
            .then(r => r.json())
            .then(data => {{
                alert(data.message || data.error);
        ...
        function fetchActiveLogs() {{
            if (!activeModalProjectId) return;
            fetch('/api/project/logs', {{
                method: 'POST',
                headers: {{ 'Content-Type': 'application/json' }},
                body: JSON.stringify({{ project_id: activeModalProjectId }})
            }})
            .then(r => r.json())
            .then(data => {{
                const el = document.getElementById('modal-logs-output');
                el.textContent = data.logs;
                el.scrollTop = el.scrollHeight; // Auto-scroll to latest log entries
            }});
        }}

        function installPackage() {{
            const pkg = document.getElementById('modal-pkg-name').value;
            if (!pkg) return;
            document.getElementById('modal-pkg-name').value = 'Processing install...';
            fetch('/api/project/pkg/install', {{
                method: 'POST',
                headers: {{ 'Content-Type': 'application/json' }},
                body: JSON.stringify({{ project_id: activeModalProjectId, package: pkg }})
            }})
            .then(r => r.json())
            .then(data => {{
                alert(data.message || data.error);
                document.getElementById('modal-pkg-name').value = '';
                fetchProjects();
            }});
        }}

        function uninstallPackage() {{
            const pkg = document.getElementById('modal-pkg-del-name').value;
            if (!pkg) return;
            document.getElementById('modal-pkg-del-name').value = 'Processing removal...';
            fetch('/api/project/pkg/uninstall', {{
                method: 'POST',
                headers: {{ 'Content-Type': 'application/json' }},
                body: JSON.stringify({{ project_id: activeModalProjectId, package: pkg }})
            }})
            ...
            .then(data => {{
                alert(data.message || data.error);
                document.getElementById('modal-pkg-del-name').value = '';
                fetchProjects();
            }});
        }}

        // Handle scoped CLI interactions
        function handleModalCliCommand(e) {{
            if (e.key === 'Enter') sendModalCliCommand();
        }}

        function sendModalCliCommand() {{
            const input = document.getElementById('modal-cli-input');
            const cmd = input.value.strip ? input.value.strip() : input.value;
            if (!cmd) return;
            
            const out = document.getElementById('modal-cli-output');
            out.innerHTML += `<p class="text-[#38bdf8] monospace">$ ${{cmd}}</p>`;
            out.scrollTop = out.scrollHeight;
            input.value = '';

            fetch('/api/project/cli', {{
                method: 'POST',
                headers: {{ 'Content-Type': 'application/json' }},
                body: JSON.stringify({{ project_id: activeModalProjectId, command: cmd }})
            }})
            .then(r => r.json())
            .then(data => {{
                if (data.stdout) out.innerHTML += `<pre class="text-green-400 whitespace-pre">${{data.stdout}}</pre>`;
                if (data.stderr) out.innerHTML += `<pre class="text-red-400 whitespace-pre">${{data.stderr}}</pre>`;
                if (data.error) out.innerHTML += `<pre class="text-red-500 font-bold">${{data.error}}</pre>`;
                out.scrollTop = out.scrollHeight;
            }});
        }}

        // Direct System Shell Commands
        function handleVpsCommand(e) {{
            if (e.key === 'Enter') sendVpsCommand();
        }}

        function sendVpsCommand() {{
            const input = document.getElementById('vps-cli-input');
            const cmd = input.value;
            if (!cmd) return;
            
            const out = document.getElementById('vps-cli-output');
            out.innerHTML += `<p class="text-cyan-400 monospace">root@vps:~# ${{cmd}}</p>`;
            out.scrollTop = out.scrollHeight;
            input.value = '';

            fetch('/api/system/cmd', {{
                method: 'POST',
                headers: {{ 'Content-Type': 'application/json' }},
            ...
            .then(r => r.json())
            .then(data => {{
                if (data.stdout) out.innerHTML += `<pre class="text-[#10b981] whitespace-pre">${{data.stdout}}</pre>`;
                if (data.stderr) out.innerHTML += `<pre class="text-yellow-500 whitespace-pre">${{data.stderr}}</pre>`;
                if (data.error) out.innerHTML += `<pre class="text-red-500 font-bold">${{data.error}}</pre>`;
                out.scrollTop = out.scrollHeight;
            }});
        }}

        // Scan Foreign Host VPS Services
        function scanVPSProcesses() {{
            const list = document.getElementById('vps-processes-list');
            list.innerHTML = '<p class="text-xs text-yellow-400 text-center py-8 animate-pulse">Running process reconnaissance scan across server host memory...</p>';
            fetch('/api/system/processes')
                .then(r => r.json())
                .then(data => {{
                    list.innerHTML = '';
                    if (data.length === 0) {{
                        list.innerHTML = '<p class="text-xs text-[#9ca3af] text-center py-8">✅ No external APIs or unlisted daemons active.</p>';
                        return;
                    }}
                    data.forEach(p => {{
                        const el = document.createElement('div');
                        el.className = "p-3 bg-[#111827] border border-[#1f2937] rounded-xl flex items-center justify-between gap-4 text-xs";
                        el.innerHTML = `
                            <div class="space-y-1">
                                <p class="font-bold text-white flex items-center gap-1.5">⚡ PID: \${p.real_pid} <span class="text-[9px] px-1.5 py-0.5 rounded bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 font-medium">\${p.project_type}</span></p>
                                <p class="text-[10px] text-[#9ca3af] monospace truncate max-w-xs">\${p.cmdline}</p>
                                \${p.port ? `<p class="text-[10px] text-cyan-400">Listening Port: \${p.port}</p>` : ''}
                            </div>
                            <button onclick="killVPSProcess(\${p.real_pid})" class="px-2.5 py-1.5 bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/30 font-bold rounded-lg transition text-[10px]">Kill PID</button>
                        `;
                        list.appendChild(el);
                    }});
                }});
        ...
        // File Selection Deployment Hub
        function handleFileSelect(e) {{
            const file = e.target.files[0];
            if (!file) return;
            selectedFileName = file.name;
            document.getElementById('file-label-text').innerText = `Selected file: ${{file.name}} (Reading...)`;
            ...
            reader.onload = function(evt) {{
                ...
                selectedFileBase64 = window.btoa(binary);
                document.getElementById('file-label-text').innerHTML = `✅ Selected file: <b>${{file.name}}</b> (Load Success)`;
            }};
        ...
        function deployProject() {{
            ...
            if (!name) {{
                alert('Please specify a project identifier name.');
                return;
            }}
            if (!selectedFileBase64) {{
                alert('Please upload your code .zip or script file.');
                return;
            }}
            const btn = document.querySelector('#viewport-deploy button');
            btn.innerText = 'Uploading build modules to sandbox...';
            btn.disabled = true;
            ...
            .then(data => {{
                alert(data.message || data.error);
                btn.innerText = 'DEPLOY TO HOST SANDBOX';
                btn.disabled = false;
                
                if (data.success) {{
                    document.getElementById('dep-name').value = '';
                    document.getElementById('dep-file').value = '';
                    document.getElementById('file-label-text').innerText = 'Drag & drop your code .zip or script file here, or click to browse';
...
# ===== MULTI-THREADED SOCKET SERVER LAUNCHER =====
class ThreadingHTTPServer(socketserver.ThreadingMixIn, http.server.HTTPServer):
    """Resilient thread-safe HTTP server allowing multiple concurrent websocket simulations"""
    daemon_threads = True

def run_web_server():
    """Spawns standard HTTP Socket servers handling retry ports if active configurations exist"""
    global WEB_DASHBOARD_PORT
    server_address = ('', WEB_DASHBOARD_PORT)
    
    for i in range(15):
        try:
            httpd = ThreadingHTTPServer(server_address, DashboardHTTPHandler)
            logger.info(f"🟢 Web Control Dashboard is online on http://localhost:{WEB_DASHBOARD_PORT}")
            httpd.serve_forever()
            return
        except OSError:
            logger.warning(f"⚠️ Port {WEB_DASHBOARD_PORT} in use. Incrementing allocation index...")
            WEB_DASHBOARD_PORT += 1
            server_address = ('', WEB_DASHBOARD_PORT)
            
    logger.error("❌ Web Orchestrator Panel failed to acquire system network bindings.")
...
def main():
    ...
    # Start Web Dashboard Server Background Thread
    ...
    app.add_handler(CommandHandler("dashboard", web_dashboard_command))
